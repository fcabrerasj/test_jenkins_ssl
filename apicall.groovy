    ...
    stages {
        ...
        stage('Upload as Secrets') {
            steps {
                ...
                // Replace <KUBE_CONFIG> with the path to your Kubernetes configuration file
                sh "kubectl --kubeconfig=<KUBE_CONFIG> login"
...
def getKeyfactorCertificates() {
    ...
    def response = sh(
        script: "curl -X GET <KEYFACTOR_API_ENDPOINT>",
        returnStdout: true
    )
...
def renewCertificate(serialNumber) {
    ...
    sh "curl -X POST <CERT_RENEWAL_API_ENDPOINT> -d 'serialNumber=${serialNumber}'"
}
...
def uploadCertificateAsSecret(serialNumber) {
    ...
    sh "kubectl --kubeconfig=<KUBE_CONFIG> create secret generic ${serialNumber} --from-literal=certificate='${certificateContent}'"
}
